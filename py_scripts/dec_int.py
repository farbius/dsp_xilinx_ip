...
def main():
    ...
    plt.subplot(221)
    plt.plot(t/1e-6, np.real(x), '.-b')
    plt.xlabel('t,usec')
    plt.title('Input signal: time')
    plt.grid()
    
    plt.subplot(222)
    plt.plot(freqs/1e6, np.abs(xfft), '.-r')
    plt.xlabel('f,MHz')
    ...
    plt.plot(t/1e-6,     np.real(x),     '.-r',   label="input")
    plt.title('Decimated signal: time')
    plt.legend(loc="upper left")
    plt.xlabel('t, usec')
    plt.grid()
    
    plt.subplot(224)
    plt.plot(freqs_dec/1e6, np.abs(xdec_fft), '.-r')
    plt.xlabel('f, MHz')
    plt.title('Decimated signal: frequency')
    plt.grid()
    ...
    plt.figure(figsize=(10,10))
    plt.subplot(221)
    plt.plot(t/1e-6, np.real(x_usmpl), '.-b')
    plt.xlabel('t,usec')
    plt.title('Interpolated signal before filtering: time')
    plt.grid()
    
    plt.subplot(222)
    plt.plot(freqs/1e6, np.abs(xfft), '.-r')
    plt.xlabel('f, MHz')
    plt.title('Interpolated signal before filtering: frequency')
    plt.grid()
    ...
    plt.title('Interpolated signal after filtering: time')
    plt.grid()
    
    plt.subplot(224)
    plt.plot(freqs/1e6, np.abs(xfft_fir), '.-r')
    plt.xlabel('f, MHz')
    plt.title('Interpolated signal after filtering: frequency')
    plt.grid()
